...
class T5Preprocessor(Preprocessor):
    """A T5 preprocessing layer which tokenizes and packs inputs.
    ...
    This preprocessing layer will do three things:
     ...
     3. Construct a dictionary with keys `"encoder_token_ids"`,
        ...
        that can be passed directly to a T5 model.

    Args:
        tokenizer: A `keras_nlp.models.T5Tokenizer` instance.
    ...
    ```python
    preprocessor = keras_nlp.models.T5Preprocessor.from_preset("t5_base_multi")
    ...
    preprocessor = keras_nlp.models.T5Preprocessor(
        tokenizer=tokenizer,
    ...
    def call(self, x, y=None, sample_weight=None):
        ...
        if len(encoder_text) > 1 or len(decoder_text) > 1:
            raise ValueError(
                '`T5Preprocessor` requires both `"encoder_text"` and '
